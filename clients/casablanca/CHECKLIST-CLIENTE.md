...
| 5 | ✅ **Formato resuelto: 4:5 + adaptación a story.** Decidido por Serena el **27-08-2026**. Toca `FORMATOS` en `scripts/casablanca-septiembre.py` y la coordenada quemada de la franja en `pieza_c2` (hoy `(822.0, 1080.0)` para feed, línea ~360). **Antes de moverlo, leer el aviso de resolución de abajo.** | Serena | Cerrado como decisión. **Ojo: la entrega del 26-08 está en 1:1** (las 14 piezas de `out/casablanca/septiembre/` son 2250×2250 + story 2250×4000), así que el 4:5 es una ronda de trabajo pendiente, no un formato ya entregado. |
