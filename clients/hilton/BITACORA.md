## 2026-09-25 (noche) — Eli (Windows) · DT: banco de la familia para Family Time — 8 escenas × 3 formatos, pulidas y en Drive

**Qué se hizo:** se creó la familia fija de DT: mamá y papá de 38, hija de ~9 e hijo de ~7, perfil chileno (Seedream 5 Pro). Eli la aprobó. Por su pedido, las escenas finales van **sobre fotos reales del hotel**: Nano Banana Pro pone a la familia y después se pegan **sólo las personas** sobre los píxeles originales (máscara por diferencia). Salieron 8 escenas en post 4:5, story 9:16 y 16:9: check-in con cookie, llegada al lobby, cookie en la cama, guerra de almohadas, noche de película, mirando Santiago, cartas en el lobby y desayuno. En la ronda de pulido se arreglaron las piernas, la cookie gigante, los naipes, la gente y los reflejos del fondo y la identidad de la mamá, que salía pelirroja o rubia. El desayuno se rehízo porque una colega preguntó dónde estaba sentada la niña.
**Dónde quedó:** Drive `1P0w__CrAANzx20N1FL9CQ0FFvnA8is7G` (01 POST · 02 STORY · 03 HORIZONTAL · 04 PERSONAJES, 27 archivos, reemplazados por nombre). Página para compartir: https://claude.ai/artifact/WrdSbUMohhjJgiGbsoLzUf. Pipeline en `scripts/dt-familia/` (`ronda3.py generar|componer|escalar|formatos`, `retoque_local.py`, `pagina.py`); personajes en `clients/hilton/dt-familia/personajes/`; subida con `scripts/dt-familia-subir-drive.py`. Maestros y descartes en `raw/hilton/dt/familia/r3/` (ignorado por git).
**Qué sigue:** que Eli revise la versión pulida. Si pide otra escena, se agrega a `ESCENAS` en `ronda3.py`: 2 variantes, elegir, componer, escalar, formatos y subir.
**Abierto:** ¿el desayuno rehecho reemplaza al aprobado? (el anterior está guardado) · la cubeta de espumante de `HDT_65` en Family Time · el conector de Magnific sin autorizar (Space y librería de personajes, lo ve Valeria).
