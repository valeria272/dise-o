# SANTA GOTA (Patagonia de Chile) — lo que el estudio sabe de este cliente

> **Qué es este archivo.** El cerebro de la cuenta: lo que se aprendió de este cliente
> sesión tras sesión, destilado. La bitácora cuenta **qué pasó**; esto dice **qué
> sabemos**. Si la diseñadora que lleva la cuenta falta mañana, con esto (más el
> manual `CLAUDE.md` y `marca.json`) otra persona retoma sin llamar a nadie.
>
> **Vale SOLO para SANTA GOTA.** Nada de acá se copia a otra marca, ni a una hermana.
> Se alimenta en cada `/cierre` — ver `docs/MEMORIA-POR-CLIENTE.md`.
>
> Criterio: **Valeria Traverso (TV y piezas del estudio); el feed lo diseña Luis Piano, del lado del cliente, y su feed manda** · Aprueba: **Gonzalo (cliente; decide qué sale a TV)** · Estrategia de marca: **Diana Meinhardt (Brand Soul)**
> Última cosecha: **2026-09-26** · Cosechas: **2**

## 1. Quién es el cliente

Aceite de oliva extra virgen «pop» de Patagonia de Chile, que se vende como **marca cultural,
no gourmet**: joven, irreverente, gráfica. Arquetipo **La Pecadora Insolente** («Bendita sea la
gota», «Let's f\*ck with the kitchen»). Vende en su e-commerce `santagota.cl` (Shopify) y quiere
colarse en la cocina cotidiana; el aceite es sólo el comienzo (vinagres, sal, café, objetos,
eventos). Son **dos aceites distintos**: 750 ml para cocinar (tapa amarilla, etiqueta naranja) y
500 ml para aderezar y terminar (tapa lima), más latas de relleno 473 ml («usa, recarga, repite»).
Si una pieza se puede confundir con un aceite de góndola, está mala.

## 2. Cómo trabaja

| | |
|---|---|
| Quién pide / KAM | Valeria Traverso (con Claude) |
| Quién aprueba (cliente) | Gonzalo, del lado del cliente (cuenta del cliente: `patagoniadechile@gmail.com`). Internamente, Valeria aprueba cada ronda antes de que salga |
| Por dónde llega el feedback | Valeria por voz/texto en la sesión; del cliente, por correo (hilo «TVN / Santa Gota Virgen / 2026») y WhatsApp |
| Dónde se entrega | `out/santagota/<pieza>/` + Escritorio; al canal por WeTransfer. Drive: carpeta `SANTA GOTA` `1VRQ_9ggb6n1DhQbOZiYoLP0iW-imswG9` |
| Ritmo | A pedido (TV). El feed mensual lo produce Luis Piano |
| Rondas típicas | Muchas: placements de TV V1→V4 en un día (11-09); spot 3 rondas de keyframes + 5 animatics (15-09). Vuelve por **oficio** (montaje, escala, «parece IA»), no por concepto |

## 3. Identidad en corto

- **Colores (medidos 11-09):** lima `#C3D600` · lima de tapa `#7ED140` (no es fondo) · verde logo `#5C921C` · naranja `#F26513` · botella `#0E1C03` · hueso `#D2CEC5` · blanco.
- **Pares:** botella sobre lima · lima sobre botella (URL) · blanco sobre foto. **Nunca** logo a color sobre lima.
- **Tipografía:** sólo Montserrat — Bold/Black para la palabra clave + Light para el resto en la misma línea.
- **Logo:** único original `logo-color.png` 810×510 (no hay vector); planos blanco y botella derivados.
- **Formatos:** feed **4:5 1080×1350** (desde la semana del 14-09) · story 1080×1920 (llega a 941×1672) · TV: huincha 1920×216 alfa · virtual 775×1080 alfa · full 1920×1080 29,97 MXF.
- **Material real:** packshots oficiales con alfa en `public/assets/santagota/producto/` · reel de la monja (actriz real) · sesión fotográfica inicial en Drive `1CTS0v8xqdHHJQxtJ7-FAfSyr_ThIpW_f`.

## 4. Reglas firmes

- **R-01** · Santa Gota es fotografía con flash y **una sola** intervención gráfica encima (logo plano, botella en línea, aureola, plumón, logo grabado en la materia…). Si hay dos, sobra una — _manual §2, medido sobre el feed de Luis Piano 11-09 y 15-09_ · ✔×2
- **R-02** · Prohibidos los códigos del aceite tradicional: oliva, hojas de olivo, madera, beige, dorados, gourmet, degradados — _brief / `marca.json` reglas duras, 11-09; QA de cada ronda de TV_ · ✔×2
- **R-03** · Logo a color nunca sobre lima (verde sobre verde desaparece): va sobre botella, hueso, blanco o foto oscura; sobre lima, plano botella; sobre foto, plano blanco — _medición 11-09, Fase 1 TV_ · ✔×2
- **R-04** · Son **dos aceites distintos**, nunca «dos tamaños». Toda pieza o respuesta de producto lo deja claro — _revisión de cuenta 15-09; manual de redes QA v3 del cliente_ · ✔×2
- **R-05** · Precio, SKU y formato salen de `https://santagota.cl/products.json?limit=250` el mismo día, nunca de memoria — _revisión 15-09 (10 SKU, cambian)_ · ✔×1
- **R-06** · La IA no toca producto, logo ni monja. Producto sólo el packshot oficial, escalado uniforme (`Producto` recibe sólo la altura; rotar sí, estirar no) — _Fase 2 TV 11-09; spot 15-09 (etiqueta Δ < 6)_ · ✔×3
- **R-07** · Concepto y CTA van literales del brief — _brief TV 11-09 («EL ACEITE QUE LLEGÓ A REVOLUCIONAR TU COCINA.» · SANTAGOTA.CL)_ · ✔×1
- **R-08** · Naranja sólo como gesto: aureola, plumón, pastilla del CTA. Nada más — _dirección aprobada Fase 2, 11-09_ · ✔×2
- **R-09** · La monja es la del **reel** (actriz real, 24 fps), no la de las fotos del feed (IA, otra actriz). Stills: fotograma 8,708 s (foco medido) — _Fase 1–2 TV, 11-09; el cliente salió con ella a TVN, 20-09_ · ✔×2
- **R-10** · No se produce ninguna cara reconocible tipo celebridad generada con IA sin autorización escrita (derecho de imagen) — _revisión 15-09, pieza «Santo munchies»_ · ✔×1
- **R-11** · Montaje de TV: cortes secos motivados, nada de zoom/empuje para llenar, franjas de borde a borde, monja grande, end frame quieto; nada entra con fade — _pauta cerrada de Valeria V4, 11-09_ · ✔×2
- **R-12** · Menos efectos = más premium; nada debe parecer IA o stock; «menos aceite, más impacto» — _Valeria, spot V2/V3, 15-09_ · ✔×2
- **R-13** · Si una mano generada no es 100 % creíble, mejor sin mano. En rodaje: mano real + proxy + tracking — _Valeria, animatic V2, 15-09_ · ✔×1
- **R-14** · Los productos nunca «aparecen» con fade o slide: se descubren por luz, cámara o corte — _animatic V2, 15-09_ · ✔×1
- **R-15** · Antes de entregar video: hoja de contacto a 6 fps **del MP4 renderizado** (no del timeline); alfa mirado sobre gris — _V3 TV 11-09 (planos que cruzaban cortes del reel); QA §8_ · ✔×2
- **R-16** · Huincha ≤ 7,00 s → **209 cuadros** a 29,97 (210 ya son 7,007) — _brief TV, 11-09_ · ✔×1
- **R-17** · Interlínea mínima 0,98 con tildes en mayúscula (a 0,92 la Ó choca) — _error Fase 2, 11-09_ · ✔×1
- **R-18** · Para saber qué se le mandó al cliente se lee el hilo de Gmail; no se confía en qué se exportó — _21-09 (el cliente mandó una versión hecha fuera del repo)_ · ✔×1
- **R-19** · Voz: Pecadora Insolente. Tuteo, ironía hacia las reglas y nunca hacia la persona, léxico religioso profanado, sin lenguaje gourmet ni efusividad — _Brand Soul de Diana Meinhardt (07-08); QA v3 de redes_ · ✔×1
- **R-20** · La sesión fotográfica inicial sirve para gente, gesto y ambiente, **no** para producto quieto (su etiqueta es anterior al packaging actual) — _revisión 15-09_ · ✔×1

## 5. Excepciones

- **E-01** · Los bloques de color lima/naranja viven en **packaging y TV**, no en el feed (el feed es foto a sangre) — _manual §2, 11-09_
- **E-02** · En TV la aureola va **naranja** (flota sobre el set blanco del canal); en el feed, **blanca** — _Fase 1, 11-09_
- **E-03** · Carrusel de recetas 4:5: cinta lima con extremos de plumón y **un emoji chico**, aunque el manual dice «emojis en gráfica: no» — _serie del diseñador, 16-09_ · ⚠️ ver §8
- **E-04** · IA permitida sólo en placas y fondos **sin producto**; extensión de fondo sólo periférica y entregando el recorte a 720 px (a 1080 re-renderiza a la monja) — _V3 TV 11-09; spot 15-09_
- **E-05** · En el full de la monja que salió al aire (hecho fuera del repo) no se corrigió la botella verde ensanchada que reclamó Gonzalo el 15-09, porque no se pidió — _21-09_
- **E-06** · Master de TV sin música (derechos; los ejemplos del canal son voz del programa), pero con pista PCM y sound design — _Fase 2 y spot, 11-09 / 15-09_

## 6. Lo que se aprueba a la primera

- **A-01** · «La monja se mete en la tele»: campo lima, tinta botella, naranja sólo en aureola y plumón, bloque botella con logo a color + URL — _Fase 1 KVs, 11-09 (aprobada con cambios)_
- **A-02** · El full de 18 s con la monja y cierre lima («…REVOLUCIONAR TU COCINA» → logo → «COMPRA EN TODO CHILE / EN SANTAGOTA.CL») es la versión que Gonzalo eligió y mandó a TVN — _20-09_
- **A-03** · Cierre de Instagram agregado sin alargar los 18 s: «SÍGUENOS EN / INSTAGRAM / @SANTAGOTA.CL» con la misma mecánica del original — _`FullClienteIG.tsx`, 21-09 (entregado)_
- **A-04** · Spot: keyframes V2 aprobados en concepto (8,5/10) y V3 aprobada — _15-09_
- **A-05** · El recurso más fuerte del feed publicado: **el logo grabado en la materia** (tallado en el pan, rapado en la nuca, en los dientes, en la polera) — _feed de Luis Piano, leído 15-09_

## 7. Lo que se rechaza

- **X-01** · Columna 9:16 sobre placa desenfocada, «pantallitas» de video en el virtual, monja chica (62 %), texto que entra con fade — _Fase 2 TV, 11-09; costó la V3_
- **X-02** · Zooms para llenar, planos que cruzan los cortes del reel, franja que no llega al borde (parecía lower third), «plato gigante» congelado, látigo lima — _V3 TV, 11-09; costó la V4_
- **X-03** · Sucesión de beauty shots: «tenemos una idea pero no un MOMENTO» — _spot keyframes V1, 15-09_
- **X-04** · Plano que parece lava y no comida; botella sola; persona con la botella en primer plano «muy falsa» — _animatics V3–V4, 15-09_
- **X-05** · La ruta «UNA GOTA. CAMBIA TODO.» completa (3 rondas de keyframes + 5 animatics + pantalla final): el cliente no la mandó («este no me tincó») — _20-09_
- **X-06** · Mano generada que deformaba el packaging — _spot V1/animatic, 15-09_
- **X-07** · Música trap/808 (irregular) — _animatic V4, 15-09_
- **X-08** · Concluir que no había packshots mirando las miniaturas del sitio; citar como «Instagram» un pantallazo que era el e-commerce — _11-09 → corregido 15-09_

## 8. Preguntas abiertas

- **Logo vectorial:** no existe (sólo el PNG de 36 KB). Pedírselo al cliente (Valeria → Gonzalo).
- **Plantilla técnica del virtual 775×1080** del canal: nunca llegó. No se entrega al canal sin calzarla.
- **¿Salió al aire?** Está verificado que se **mandó** a TVN el 20-09; que esté «al aire» no está confirmado. No decirlo en ninguna pieza (Valeria / Gonzalo).
- ¿La monja vuelve a estar permitida en TV? El 15-09 el cliente la había dejado sólo para RRSS y el 20-09 mandó justo la versión con monja (Valeria / Gonzalo).
- ¿Huincha y virtual de la versión que salió? Sólo existe el full; el proyecto del otro agente no está versionado.
- ¿Se corrige la botella ensanchada del full emitido?
- ¿Emojis en gráfica sí o no? El manual dice no; la serie de recetas del diseñador usa uno (Luis Piano / Valeria).
- ¿El feed se sigue haciendo con IA o se explota la sesión real ya pagada? (Valeria).
- `SANTA GOTA SOCIAL MEDIA MANAGEMENT.pdf` (81 MB, sin capa de texto) sin leer.
- Obra sin commitear al 22-09 (spot y full con Instagram): mientras no entre al repo nadie más la reproduce.

## 9. Registro de cosechas

### 2026-09-26 — Claude nocturno (nube) · revisión de rutina, sin sesiones nuevas
- sin aprendizajes nuevos: los 4 commits de TV (`2ca00d1` Fase 1, `bd7f0fa` Fase 2, `5cc4249` V3, `4886fb0` V4) y la parte de Santa Gota del commit `66b38a1` (cierre del 21-09 con el ícono de Instagram en el full, y las 6 rondas del spot «UNA GOTA. CAMBIA TODO.» del 15-09) ya están destiladas en la siembra inicial de abajo — R-11 a R-16, A-01 a A-03, E-05, E-06 y X-01 a X-08 cubren ese material aunque los commits se autoraron el 11 y el 15-09 (llegaron a git recién el 25-09, con la fecha de commit reescrita, pero ya estaban en el árbol de trabajo cuando se sembró este archivo). El resto de `66b38a1` (feed Copywriters, Petra, G.CL) no es de esta marca.

### 2026-09-25 — Claude (siembra inicial) · destilado del manual, la bitácora y el feedback histórico
- nuevo **R-01…R-20** · sembradas desde `clients/santa-gota/` (CLAUDE.md, BITACORA.md del 11-09 al 21-09, marca.json) y la memoria `santa-gota-tv-placements`.
- nuevo **X-01…X-08** · las rondas de TV del 11-09 y del spot del 15-09; la ruta sin monja entera queda como rechazo del cliente.
- nuevo **A-02** · lo que manda es lo que Gonzalo eligió para TVN (la monja), no la última ruta del estudio.
- abierto · emojis en gráfica, «al aire» sin confirmar y el ida y vuelta de la monja en TV (§8).
