...
> **Este documento manda sobre todos los demás del pack.** El V4 se escribió
> antes que estas decisiones; donde el V4 diga otra cosa, gana lo que está acá.
>
> Un candado no se discute: se aplica. Si hay que cambiarlo, se agrega un candado
> nuevo con su fecha y se marca el viejo como revocado. **Nunca se borra** —
> el registro de por qué el universo es como es vale tanto como el universo.
...
## LOCK 1 · El protagonista se llama G

**G.** Nada más.

«Gigi» **no existe**. No es un apodo, no es un nombre alternativo, no es como le
dicen en confianza y no aparece en diálogo. Fuera del universo entero.

Se eliminó de los **17 documentos del pack** (68 mentions), de la carpeta
`03_GIGI_CREW` → `03_G_CREW` y del archivo `GIGI_PERSONALITY.md` →
`G_PERSONALITY.md`.

> **Por qué importa más de lo que parece.** Un personaje con dos nombres es un
> personaje que el equipo escribe de dos maneras, y a los seis capítulos son dos
> personajes. Además «Gigi» lo ablanda hacia la mascota, que es justo lo que el
> propio V4 prohíbe («never a cute Pixar-like mascot»). El nombre de una letra es
> parte de por qué funciona: es corto, es seco y es un logo.

---

## LOCK 2 · La jerarquía del universo

| | |
|---|---|
| **G.C.L.** | el universo · la serie · el Departamento de Cosas Imposibles |
| **G** | el protagonista |
| **Nivel -1** | la base principal de operaciones de G.C.L. |
| **Marta · R.01 · Server** | los colaboradores recurrentes de G |
| **Copywriters** | el mundo humano y real que existe arriba y alrededor |

La consecuencia práctica: **`G.C.L.` nunca nombra al personaje.** Es la marca
gráfica, la serie y el departamento. Cuando en pantalla aparece `G.C.L.` se está
firmando el universo, no presentando a alguien. Y `Copywriters` no es el
decorado de G.C.L.: es un mundo real que existe por su cuenta y donde G.C.L.
vive abajo.

---

## LOCK 3 · Las cifras eran orientación, no reglas

**Se revocan los números del V4:**

| Revocado | Dónde estaba |
|---|---|
| `60–70 %` Nivel -1 · `15–25 %` misiones · `10–15 %` humanos | `08_SOCIAL_SYSTEM/SERIES_SYSTEM.md` |
| Valeria `1 de cada 6` piezas | `VALERIA_DIRECTION.md` · `HUMAN_CAST_CANON.md` · `MASTER_UNIVERSE_BIBLE.md` · `MASTER_PROMPT` |

**Los humanos aparecen cuando la historia los necesita.** Un capítulo no está mal
por no tener ninguno, y tampoco por tener varios.

> Lo que sí se conserva de esas reglas es su intención, que era otra cosa: el
> centro de gravedad de la serie es **G y su tripulación**, y G.C.L. no puede
> convertirse en «Valeria y su robot». Eso sigue en pie. Lo que se cae es
> contarlo con una calculadora.
