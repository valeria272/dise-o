#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Página de revisión de la RONDA 10 del carrusel CONCURSO de Between (21-09-2026).
...
doble clic, sin servidor.
    ...
    python scripts/between-concurso-s3-r10-revision.py
"""
import base64
import io
import sys
...
DIR = "out/hilton/between/concurso-s3-r10/"
SALIDA = RAIZ / DIR / "revision-r10.html"
...
def main() -> int:
    ...
    partes.append("""
<section>
  <h2>Los tres cambios</h2>
  <blockquote>&laquo;Este invierte el color, el texto caf&eacute; y el fondo beige. Agranda un poco el texto
    y caja del CEO. M&aacute;s deja <b>puntas rectas</b> al de caja del CEO. Y listo.&raquo;
    <small>Eli &middot; 21-09</small></blockquote>
  <table>
    <tr><th>Qu&eacute;</th><th>Antes</th><th>Ahora</th></tr>
    <tr><td>Caja &laquo;1 MES DE CAF&Eacute; GRATIS&raquo;</td><td>taupe con tinta beige</td>
        <td><b>beige con tinta caf&eacute;</b></td></tr>
    <tr><td>Cuerpo del CEO</td><td class="n">54</td><td class="n"><b>60</b></td></tr>
    <tr><td>Caja del CEO</td><td class="n">78 &times; 385</td><td class="n"><b>88 &times; 456</b></td></tr>
    <tr><td>Puntas de la caja del CEO</td><td>radio 16</td><td><b>rectas</b></td></tr>
  </table>
  <p class="que" style="margin-top:16px">La inversi&oacute;n ordena algo m&aacute;s: con las dos cajas del mismo
     taupe ninguna mandaba sobre la otra &mdash;es el defecto que la ronda 5 ya hab&iacute;a diagnosticado en
     el sello&mdash; y ahora el bloque macizo es el del titular, que es el nivel de arriba. Las puntas
     rectas tampoco son un capricho: el bloque de color de la REF 1 no tiene radio. El radio de la
     marca se queda en las cajas de dato, que s&iacute; lo llevan.</p>
</section>""")

    d_antes = det(ANTES, (100, 400, 980, 760), "<b>ANTES</b>", 1.2)
    d_fin = det(FINAL, (100, 400, 980, 760), "<b>AHORA</b>", 1.2)
    partes.append("""
<section class="elige">
  <h2>El bloque, antes y despu&eacute;s</h2>
  <div class="dos">
    %s
    %s
  </div>
  <div class="notas">
    <h3>De d&oacute;nde salieron los 16 px que necesitaba la caja</h3>
    <ul>
      <li>La caja creci&oacute; de 96 a 112 px de alto contando la inclinaci&oacute;n, y el hueco entre la bajada
          y la pila no da para m&aacute;s. Se apretaron los aires que <b>no</b> est&aacute;n atados a una
          proporci&oacute;n: el de arriba del r&oacute;tulo (25&rarr;21), el de la bajada al bloque (25&rarr;20) y el
          interno del bloque (30&rarr;10 &mdash; que adem&aacute;s es lo que hace la REF, donde la caja monta
          sobre el texto).</li>
      <li><b>El aire r&oacute;tulo&rarr;bajada se qued&oacute; en 32</b>: es el &uacute;nico que est&aacute; atado a una
          proporci&oacute;n medida (1,5&times; el aire entre las l&iacute;neas de la bajada) y ése no se negocia.</li>
      <li>Entre la caja del CEO y &laquo;&iquest;El sueldo?&raquo; quedan <b>23 px</b>, ya con la rotaci&oacute;n
          descontada.</li>
    </ul>
  </div>
...
<section>
  ...
  <table>
    ...
    <tr><td>Caja del CEO contra el papel</td><td class="n ok">4,19 : 1</td><td>tinta por dentro 6,31 : 1</td></tr>
    <tr><td>Caja del premio contra el papel</td><td class="n">1,50 : 1</td>
        <td>tinta por dentro <b>6,31 : 1</b> &mdash; se lee por su tinta, no por su canto</td></tr>
    <tr><td>CONCURSO contra el papel</td><td class="n ok">4,14 : 1</td><td>sin cambios</td></tr>
    <tr><td>Bajada</td><td class="n ok">4,16 : 1</td><td>sin cambios</td></tr>
...
<section>
  ...
  <div class="notas">
    ...
    <ul>
      <li>Empaquetado con el nombre del portal: <code>C1 S3 CONCURSO N1.png</code> y
          <code>N2.png</code>, 2250&times;2812 a 150 ppp, en
          <code>out/hilton/between/entrega-c1-s3-concurso/</code>.</li>
      <li><b>No sub&iacute; nada.</b> Con tu OK las subo reemplazando el contenido de los archivos que ya
          est&aacute;n en Drive &mdash; <b>los enlaces de la grilla no cambian</b>.</li>
    ...
    cuerpo = ("<title>Concurso Between &middot; Cierre</title>\n<style>" + CSS + "</style>\n"
              """<div class="wrap">
<header>
  <div class="tag">BETWEEN &middot; CARRUSEL CONCURSO &middot; RONDA 10</div>
  <h1>Las dos cajas, invertidas</h1>
  <p class="sub">21-09-2026 &middot; FEED del <b>24 de septiembre</b> (S4) &middot; carrusel terminado y
     empaquetado</p>
</header>
""" + ''.join(partes) + """
<footer>P&aacute;gina generada por <code>scripts/between-concurso-s3-r10-revision.py</code>.</footer>
</div>""")
    ...
    ART = SALIDA.with_name("revision-r10-artefacto.html")
