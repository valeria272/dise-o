#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ST EMERGENCIA BETWEEN (S2) — ronda 17: la caja se GENERA, y los productos se revelan.
...
Salida: public/assets/hilton/between/ia-sept/emergencia-fondo-r17.png (2250×4000)
"""
import sys
...
SALIDA = BASE / "ia-sept/emergencia-fondo-r17.png"
...
FACTOR = (2250 / 3072) * 0.965
# ⚠️ DESPL_Y calculado, no tanteado. El MARCO de la generación va de y=780 a
# y=4560, o sea 3780 px → 2672 al 0,7068 de factor. La zona útil de Meta en una
# historia es y 558..3292 del lienzo, o sea **2734 px: el marco cabe justo**, con
# 31 px de margen arriba y abajo si se centra. Su centro tiene que caer en 1925:
#     780 · 0,7068 + DESPL_Y = 1925 − 2672/2  →  DESPL_Y = 38
# A −74 el marco se salía 14 px por ARRIBA y a −16 se salía 16 px por abajo.
DESPL_Y = 38
DESPL_X = 39
...
def g(v):
    ...
    return int(round(v * FACTOR)) + DESPL_Y


def gx(v):
    return int(round(v * FACTOR)) + DESPL_X
...
PRODUCTOS = [
    ("togo-vaso-real-nobg.png",              730, 1140, -60),
    ("recortes/croissant-h3-nobg.png",       620,  830,  60),
    ("recortes/muffin-chocolate-limpio.png", 430, 1490,  26),
]
...
def fondo():
    """La caja generada, compuesta sobre un muro del mismo crema.

    El muro se rellena con el color medio de la propia pared de la generación
    —así no hay salto de tono en el empalme— y la caja se pega con su sombra
    incluida, desplazada según DESPL_X/DESPL_Y."""
    im = Image.open(CAJA).convert("RGB")
    a = np.asarray(im).astype(np.float32)
    L = a[..., 0] * 0.299 + a[..., 1] * 0.587 + a[..., 2] * 0.114
    pared = a[L > 200]
    crema = tuple(int(v) for v in pared.mean(axis=0)) if len(pared) else (238, 228, 214)
    an = int(round(im.width * FACTOR))
    al = int(round(im.height * FACTOR))
    im = im.resize((an, al), Image.LANCZOS)
    lienzo = Image.new("RGB", (W, H), crema)
    lienzo.paste(im, (DESPL_X, DESPL_Y))
    print(f"   muro crema {crema} · caja {an}×{al} en ({DESPL_X},{DESPL_Y})")
    return lienzo
...
def pon_producto(lienzo, ruta, ancho, cx, adelanto):
    ...
    rx, ry = int(an * 0.54), 26
    dc.ellipse([x + an // 2 - rx, base_y - ry, x + an // 2 + rx, base_y + ry],
               fill=215)
    con = con.filter(ImageFilter.GaussianBlur(17))
...
def main():
    ...
    lienzo.resize((W // 3, H // 3), Image.LANCZOS).save(
        PASOS / "emergencia-r17.jpg", quality=90)
