#!/usr/bin/env python3
"""CAP. 02 «ES UN CAMBIO CHICO» — CLARITY CUT V3 · 1364 frames = 45,5 s


    VOZ_G=D /Users/Vale/copylab-venv/bin/python3 scripts/cap02-audio-v3.py
...
SALIDA = RAIZ / "public/assets/gcl/cap02/v3_audio.wav"
...
poner(zumbido_server(f2s(911 - 72)), 72)
poner(zumbido_server(f2s(FRAMES - 911), gain=0.005), 911)
...
# ── CONSECUENCIA 6 · PAUTA · 737–760 → 10b · 761–790 · R.01 arrastra la lista ─
compas(737, 801, gain=1.1, capas=4)
poner(sello_golpe(), 737); poner(ping(1046, 0.25) * 0.8, 740); poner(ping(1318, 0.2) * 0.6, 748)
poner(papel_continuo(f2s(30), gain=0.35), 761)
poner(cabezal_marta(f2s(30), lineas=6, gain=0.5), 761)
poner(motor_r01(f2s(30), gain=0.07), 761)

# ── SATURACIÓN · ráfaga · 791–826 ────────────────────────────────────────────
compas(801, 827, gain=1.15, capas=4)
poner(cabezal_marta(f2s(12), lineas=3, gain=1.2), 791)
poner(motor_r01(f2s(12), gain=0.09), 803)
for f in (815, 819, 823): poner(ticker_tick() * 1.6, f)

# ── CONSECUENCIA 7 · PRESENTACIÓN · 827–850 ──────────────────────────────────
compas(827, 891, gain=1.15, capas=4); compas(891, 911, gain=1.15, capas=4)
poner(sello_golpe() * 1.1, 827); poner(ping(880, 0.3) * 0.8, 830)

# ── COLAPSO ×3 · 851–910 · cada uno con su muerte eléctrica ──────────────────
poner(cabezal_marta(f2s(10), lineas=3, gain=1.0), 851); poner(muerte_electrica(0.5) * 0.8, 860)   # Marta
poner(motor_r01(f2s(10), gain=0.08), 871); poner(r01_frena(0.25) * 0.7, 878); poner(muerte_electrica(0.5) * 0.8, 882)   # R.01
poner(zumbido_server(f2s(12), gain=0.03, hz=130), 891); poner(muerte_electrica(0.7), 898)   # Server

# ── EL DESASTRE · 911–1045 · SILENCIO A MUESTRA CERO · sólo G: tic-tic-tic ───
i0 = int(round(f2s(911) * SR)); pista[i0:] = 0
poner(zumbido_server(f2s(FRAMES - 911), gain=0.004), 911)
f = 916
while f < 1040:
    poner(tap() * 0.55, f); f += 5 if (f // 30) % 3 else 7                 # teclea, con dos respiros
poner(hoja_desliza(0.3) * 0.5, 960)                                       # una hoja resbala de la pila
poner(lp(n_ruido(int(0.2 * SR)), 900) * env(int(0.2 * SR), 0.01, 0.15, 5) * 0.12, 1005)   # el parpadeo del Server

# ── 14 · 1046–1113 · tsh-TUNK · «una cosita más…» ────────────────────────────
poner(tsh_tunk(), 1046)

# ── NO COLECTIVO · 1114–1183 ─────────────────────────────────────────────────
poner(cabezal_marta(f2s(6), lineas=1, gain=1.1), 1114)                    # Marta: NO.
poner(ticker_tick() * 1.8, 1128); poner(ticker_tick() * 1.8, 1132)        # Server: NO.
poner(r01_frena(0.35) * 0.9, 1142)                                        # R.01: frena
poner(cabezal_marta(f2s(6), lineas=2), 1154)                              # la hoja: NO.
poner(lp(n_ruido(int(0.12 * SR)), 1500, 2) * env(int(0.12 * SR), 0.001, 0.08, 8) * 0.25, 1156)

# ── G SE ACTIVA · 1184–1303 · lee · visor +15 % (ping) · teclea acelerando · CUT ─
poner(ping(2093, 0.35) * 0.9, 1208)                                       # el visor se enciende
poner(tick_visor() * 1.2, 1208)
f = 1224; paso_f = 7.0
while f < 1304:
    poner(tap() * 0.7, int(f)); f += paso_f; paso_f = max(2.5, paso_f * 0.94)   # tic-tic-tic cada vez más rápido
# ── firma · 1304 ─────────────────────────────────────────────────────────────
poner(nota_grave(f2s(60)), 1304)
