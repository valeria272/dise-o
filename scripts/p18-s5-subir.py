#!/usr/bin/env python3
"""Sube la S5 de PISO18 a su carpeta del Drive, con la misma estructura de la S4.

    python scripts/p18-s5-subir.py --dry-run
    python scripts/p18-s5-subir.py
...
Estructura que deja:
    ...
    S5 HILTON SEP 2026 / PISO18 /
        ├── REF/            (ya estaba: las 2 referencias que subió Eli el 15-09)
        ├── C1 S5 PISO18/   C1 S5 N°1..3.png      (carrusel 29-09, 2250×2813)
        └── STS/            ST N°1 S5.png          (historia 28-09, 2250×4000)
                            ST N°2 S5.png          (historia 30-09, 2250×4000)

⭐ La carpeta de destino **la creó Eli el 15-09 a las 19:49Z**, el mismo minuto en
que subió las dos referencias a `REF`. No se inventa ninguna ubicación.

⚠️ La numeración de las historias es LA SUYA: nombró las referencias `ref st n°1
s5` y `ref st n°2 s5`, así que ST 1 es la del 28-09 y ST 2 la del 30-09.
...
# S5 HILTON SEP 2026 › PISO18 — carpeta creada por Eli el 15-09-2026
PISO18_S5 = "1xVz5CF-tR2qZgwnjSrma-RLQNxRBKdDY"
ENTREGA = pathlib.Path(str(_RAIZ / "out/piso18/s5/entrega"))
...
def main():
    ...
    for f in sorted(ENTREGA.iterdir()):
        if f.is_file():
            ...
            sube(svc, f, PISO18_S5, a.dry_run)
