#!/usr/bin/env python3
"""V5 — UNA sola canción (A-garage-1, 130 bpm) de punta a punta, sin empalmes.
...
SR=48000; DUR=22.5; N=int(SR*DUR); OFF=1.52
...
v0,v1=9.1,9.5; g[s2i(v0):s2i(v1)]=0; g[s2i(v0-0.02):s2i(v0)]*=np.linspace(1,0,s2i(0.02))   # el único vacío
g[s2i(v1):s2i(v1+0.004)]*=np.linspace(0,1,s2i(0.004))
g[s2i(22.47):]*=np.linspace(1,0,N-s2i(22.47))                    # corte seco
mix*=g[:,None]; peak=np.abs(mix).max(); mix*=(10**(-1/20))/max(peak,1e-9)
wav=os.path.join(AU,'banda-v5.wav'); mp3=os.path.join(AU,'banda-v5.mp3')
