...
SR=48000; DUR=19.6; N=int(SR*DUR); OFF=4.42
...
v0,v1=6.15,6.6; g[s2i(v0):s2i(v1)]=0; g[s2i(v0-0.02):s2i(v0)]*=np.linspace(1,0,s2i(0.02))   # el único vacío
g[s2i(v1):s2i(v1+0.004)]*=np.linspace(0,1,s2i(0.004))
g[s2i(19.57):]*=np.linspace(1,0,N-s2i(19.57))                    # corte seco
mix*=g[:,None]; peak=np.abs(mix).max(); mix*=(10**(-1/20))/max(peak,1e-9)
wav=os.path.join(AU,'banda-v6.wav'); mp3=os.path.join(AU,'banda-v6.mp3')
