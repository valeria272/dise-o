// ============================================================================
// G.C.L. / CAP.02 — «ES UN CAMBIO CHICO» · CLARITY CUT V3 · FINAL DE CLARIDAD
// 1364 frames · 45,5 s · 30 fps · 1080×1920 · STORY LOCK V1.6 · 06-09-2026
// ----------------------------------------------------------------------------
...
//
// V3 (06-09, corrección final de claridad de Valeria): fuera el ascensor, el
// guante gigante y las piernas humanas — nada que obligue a inventar lore.
// R.01 arrastra la lista hasta quedar atrapado; los tres colaboradores colapsan
// (Marta ahogada, R.01 detenido, Server apagándose); G sigue tic-tic-tic.
// Final: desastre en silencio → tubo → «una cosita más…» → NO colectivo
// (Marta / Server / R.01 / la hoja) → G lee, se activa feliz y se pone a
// trabajar → CUT. Dos planos nuevos generados (S17 desastre, S18 G se activa).
//
// CANON (V2): ningún plano nuevo. Todo es reedición, recorte, hold, inserto y post.
...
/** El +15 % del visor cuando G se activa: un velo rosado en screen. */
const VisorMas: React.FC<{x: number; y: number; r: number; p: number}> = ({x, y, r, p}) => (
  <div style={{position: "absolute", left: x - r, top: y - r, width: r * 2, height: r * 2, borderRadius: "50%",
    background: `radial-gradient(circle, ${ROSA}66 0%, transparent 70%)`, opacity: p * 0.9, mixBlendMode: "screen"}} />
);
...
export const V3_FRAMES = 1364;

export const Cap02ClarityCutV3: React.FC = () => {
  asegurarFuentes();
  const f = useCurrentFrame();
  return (
    <AbsoluteFill style={{backgroundColor: "#000"}}>
      <Audio src={staticFile("assets/gcl/cap02/v3_audio.wav")} />
      ...
      <Sequence from={217} durationInFrames={18}><Clip src={b2("s05_bandeja.mp4")} desdeS={0.3} /></Sequence>   {/* V3: antes de que entre el guante (1,2 s) */}
      <Sequence from={235} durationInFrames={42}><Inserto src="v2_copy.png" rot={-4} /></Sequence>
      <Sequence from={277} durationInFrames={12}><Clip src={b1("s02a_marta.mp4")} desdeS={2.4} /></Sequence>   {/* V3: Marta imprime la siguiente (sin guante) */}
      ...
      <Sequence from={481} durationInFrames={66}><Clip src={b3("s08_r01_regla.mp4")} desdeS={0} /></Sequence>   {/* V3: R.01 llega con la regla y queda TRABADO · 0–2,2 s: el guante asoma a los 2,3 s */}
      ...
      {/* ═══ CONSECUENCIA 6 · PAUTA · 737–760 → 10b · 761–790 · R.01 arrastra la lista (sin guante) ═══ */}
      <Sequence from={737} durationInFrames={24}><Inserto src="v2_pauta.png" rot={-3} /></Sequence>
      <Sequence from={761} durationInFrames={30}><Clip src={b3("s10b_kilometro.mp4")} desdeS={0} /></Sequence>

      {/* ═══ SATURACIÓN · ráfaga · 791–826 · hoja · R.01 · RENDER OK ═══ */}
      <Sequence from={791} durationInFrames={12}><Clip src={b2("s05_bandeja.mp4")} desdeS={0.5} /></Sequence>
      <Sequence from={803} durationInFrames={12}><Clip src={b3("s10b_kilometro.mp4")} desdeS={0.3} escala={1.25} /></Sequence>
      <Sequence from={815} durationInFrames={12}>
        <Clip src={b3("s09_ticker.mp4")} desdeS={2.5} />
        <Ticker texto={f - 815 < 6 ? "VIDEO · RENDER 9/9" : "VIDEO · OK"} />
      </Sequence>

      {/* ═══ CONSECUENCIA 7 · PRESENTACIÓN · 827–850 ═══ */}
      <Sequence from={827} durationInFrames={24}><Inserto src="v2_presentacion.png" rot={-2} /></Sequence>

      {/* ═══ COLAPSO ×3 · 851–910 · Marta ahogada · R.01 detenido · Server se apaga · 20 f cada uno ═══ */}
      <Sequence from={851} durationInFrames={20}>
        <Clip src={b3("s15_no.mp4")} desdeS={0.2} />
        <LCD linea1={(f - 851) % 8 < 4 ? "AY." : ""} caja={{x: 250, y: 640, w: 170, h: 62}} />
      </Sequence>
      <Sequence from={871} durationInFrames={20}><Clip src={b3("s08_r01_regla.mp4")} desdeS={1.4} /></Sequence>   {/* R.01 ya no lo intenta · 1,4–2,07 s */}
      <Sequence from={891} durationInFrames={20}>
        <Clip src={b1("s02b_server.mp4")} desdeS={3.62} />
        <Ticker texto={(f - 891) % 6 < 3 ? "· · · · · ·" : ""} />
      </Sequence>

      {/* ═══ EL DESASTRE · 911–1045 · 4,5 s · silencio · todos rendidos, G tic-tic-tic ═══ */}
      <Sequence from={911} durationInFrames={69}><Clip src={b3("s17_desastre.mp4")} desdeS={0.2} /></Sequence>   {/* hasta 2,5 s: después la G del visor deriva a anillo */}
      <Sequence from={980} durationInFrames={66}><Clip src={b3("s17_desastre_rev.mp4")} desdeS={2.58} /></Sequence>   {/* el mismo clip al revés: G sigue tecleando */}

      {/* ═══ 14 · 1046–1113 · tsh-TUNK · «una cosita más…» 1,6 s ═══ */}
      <Sequence from={1046} durationInFrames={20}><Clip src={b1("s01_tubo.mp4")} desdeS={1.7} /></Sequence>
      <Sequence from={1066} durationInFrames={48}><PostItMacro texto={<>una<br />cosita<br />más…</>} /></Sequence>

      {/* ═══ NO COLECTIVO · 1114–1183 · Marta NO. · Server NO. · R.01 frena · la hoja NO. ═══ */}
      <Sequence from={1114} durationInFrames={14}>
        <Clip src={b1("s02a_marta.mp4")} desdeS={3.0} />
        <LCD linea1="NO." caja={{x: 250, y: 640, w: 170, h: 62}} />
      </Sequence>
      <Sequence from={1128} durationInFrames={14}>
        <Clip src={b3("s09_ticker.mp4")} desdeS={2.5} />
        <Ticker texto="NO." />
      </Sequence>
      <Sequence from={1142} durationInFrames={12}><Clip src={b1("s02c_r01.mp4")} desdeS={1.95} /></Sequence>
      <Sequence from={1154} durationInFrames={30}><Inserto src="NO.png" rot={-2} /></Sequence>

      {/* ═══ G SE ACTIVA · 1184–1303 · lee, visor +15 %, pega el post-it, teclea · CUT ═══ */}
      <Sequence from={1184} durationInFrames={120}>
        <Clip src={b3("s18_g_activa.mp4")} desdeS={0.2} />   {/* lee 0,2–1,0 · pega 1,8 · teclea desde 2,6 */}
        <VisorMas x={540} y={760} r={190} p={f - 1184 >= 24 && f - 1184 < 40 ? 1 : 0} />
      </Sequence>

      <Sequence from={1304} durationInFrames={60}><Firma /></Sequence>
      <Grano op={0.05} />
    </AbsoluteFill>
  );
};
