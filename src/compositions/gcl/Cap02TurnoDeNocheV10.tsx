// ============================================================================
// G.CL / CAP.02 — «TURNO DE NOCHE» · CORTE 10 · «mañana lo veo» (= corte 7 aprobado + música que conecta + V3)
...
export const TURNO_V10_FRAMES = PLANOS.reduce((a, p) => a + p.dur, 0);
...
// Música que conecta: UN tema (temp-g) en cuatro estados, con cross-fades largos.
//   INTRIGA (temp-h) del post-it al halo → riser + coro → el TEMA entero de los códigos a V3 LISTA
//   (muere con el sting) → el TEMA CANSADO (grave, lento, con eco) mientras sube, llega, duermen y
//   brindan → en el sorbo de Pancho, el TEMA FRESCO (su intro, un pelo más rápido) y encima la
//   comedia de oficina (temp-k) → el DING del cliente lo mata → silencio, riser, explosión seca,
//   trombón → el GOLPE FINAL del tema cae con el cierre.
const TENSION = "assets/gcl/cap02-v3/musica/temp-h-tension.wav";
const TEMA = "assets/gcl/cap02-v3/musica/temp-g-hype140.wav";
const TEMA_CANSADO = "assets/gcl/cap02-v3/musica/temp-g-cansado.wav";
const TEMA_FRESCO = "assets/gcl/cap02-v3/musica/temp-g-fresco.wav";
const TEMA_FINAL = "assets/gcl/cap02-v3/musica/temp-g-final.wav";
const OFICINA = "assets/gcl/cap02-v3/musica/temp-k-oficina.wav";
const TRAMOS_MUSICA: {src: string; desde: number; hasta: number; desdeS: number; vol: number; ini?: number; fin?: number}[] = [
  {src: TENSION, desde: 0, hasta: en("P06", 18), desdeS: 0.6, vol: 0.75, fin: 14},
  {src: TEMA, desde: en("N07", 8), hasta: en("LISTA", 10), desdeS: 2.5, vol: 0.75, fin: 8},
  {src: TEMA_CANSADO, desde: en("LISTA", 30), hasta: en("P17", 12), desdeS: 3.0, vol: 0.7, ini: 24, fin: 18},
  {src: TEMA_FRESCO, desde: en("P17"), hasta: en("RESP", 6), desdeS: 0, vol: 0.45, ini: 12, fin: 3},
  {src: OFICINA, desde: en("P17", 20), hasta: en("RESP", 6), desdeS: 0.5, vol: 0.5, ini: 20, fin: 3},
  {src: TEMA_FINAL, desde: en("CIERRE") - 12, hasta: en("CIERRE") + 78, desdeS: 0, vol: 0.8, ini: 4, fin: 20},
];

export const Cap02TurnoDeNocheV10: React.FC = () => {
  asegurarFuentes();
  return (
