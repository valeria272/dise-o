// ============================================================================
// G.CL / CAP.02 — «TURNO DE NOCHE» · CORTE 16 · DEFINITIVO = corte 15 + la canción vuelve en la tarjeta + descanso más corto
// Corte 16 (Valeria): (1) después de la explosión, la canción RETOMA desde donde la mató el ding (canción
// 83,2 s, el «turun tururun») y cierra el reel sobre la tarjeta; (2) «TURNO TERMINADO» + el café, más cortos.
// Corte 15 (Valeria: «cuando termina el trabajo, bajar la música más que apagarla, porque después vuelve y
// queda como desconectado»): el riff entero no muere con el sting; se va agachando durante 1,3 s mientras el
// riff cansado entra por debajo (cross-fade largo), así la música nunca se corta entre el sello y el descanso.
...
const PLANOS: Plano[] = [
  ...
  {id: "N11", dur: 32, desde: 1.4, vel: 1.2},
  // DESCANSAN (zzz) y CELEBRAN con café → match cut al sorbo de Pancho
  {id: "P22", dur: 28, desde: 0.5},
  {id: "N22", dur: 36, desde: 0.3, vel: 1.2},
  // 09:00 — con ritmo
...
export const TURNO_V16_FRAMES = PLANOS.reduce((a, p) => a + p.dur, 0);
...
const SONIDO: Pista[] = [
  ...
  {src: fx("sorbo"), desde: en("N22", 20), vol: 0.9},
...
//      todo el montaje hasta el sello «V3 LISTA»: ahí BAJA (no muere) y se funde con el riff cansado.
...
//   5. CIERRE: tras el negro, la canción RETOMA justo donde la mató el ding (beat 83,20 s) y cierra el reel
//      sobre la tarjeta, con el tecleo encima. Fade largo al final.
...
const TRAMOS_MUSICA: {src: string; desde: number; hasta: number; desdeS: number; vol: number; ini?: number; fin?: number; duck?: [number, number, number]}[] = [
  {src: STROKES, desde: DESDE_HOOK, hasta: en("LISTA", 44), desdeS: 0, vol: 0.55, ini: 6, fin: 40,
    duck: [en("N05") - DESDE_HOOK, en("T1") - DESDE_HOOK, 0.55]},
  {src: STROKES_CANSADO, desde: en("LISTA", 6), hasta: en("P17", 12), desdeS: 0, vol: 0.6, ini: 34, fin: 18},
  {src: STROKES, desde: en("P17"), hasta: en("RESP", 6), desdeS: 79.55 - 2 / 30, vol: 0.5, ini: 2, fin: 3},
  // el ding la mató en canción ≈ 83,28 s; retoma en el beat 83,20 s sobre la tarjeta y cierra el reel
  {src: STROKES, desde: en("NEGRO", 6), hasta: en("CIERRE") + CIERRE, desdeS: 83.20, vol: 0.55, ini: 14, fin: 36},
];

export const Cap02TurnoDeNocheV16: React.FC = () => {
  asegurarFuentes();
  return (
