// ============================================================================
// G.CL / CAP.02 — «TURNO DE NOCHE» · CORTE 6 · «mañana lo veo» (feedback del corte 5)
...
const PLANOS: Plano[] = [
  ...
  {id: "P02", dur: 84, desde: 2.2, vel: 1.0, grano: true},
  // EL TUBO ▼ — 19:02
  ...
  // DESCANSAN y CELEBRAN con café
  {id: "N22", dur: 66, desde: 0.3, vel: 1.2},
  // 09:00 — con tiempo
  {id: "P17", dur: 36, desde: 0.5, vel: 1.3, grano: true},
  {id: "MAIL", clip: "P17", dur: 84, desde: 3.5, vel: 0.2, grano: true},
  {id: "RESP", clip: "P17", dur: 66, desde: 4.2, vel: 0.15, grano: true},
  {id: "N16", dur: 60, desde: 0.2, vel: 1.6, grano: true},
  // ABAJO reciben el nuevo input → explotan
  {id: "BAJA", clip: "N02", dur: 15, desde: 0.3, vel: 2.5},
  {id: "LLEGA", clip: "N03", dur: 21, desde: 0.4, vel: 1.8},
  {id: "LEEN", clip: "N15", dur: 18, desde: 0.2, vel: 1.2},
  {id: "N13", dur: 54, desde: 1.9, vel: 1.8},
  {id: "N14", dur: 27, desde: 0.2},
  {id: "CIERRE", dur: CIERRE},
];
const INICIO: Record<string, number> = {};
{let t = 0; for (const p of PLANOS) {INICIO[p.id] = t; t += p.dur;}}
export const TURNO_V6_FRAMES = PLANOS.reduce((a, p) => a + p.dur, 0);
...
/** El pensamiento de Pancho, saliendo de su cabeza (cabeza en x 560–840, y 440–800 del clip). */
const Pensamiento: React.FC = () => {
  ...
  const burbujas = [{x: 545, y: 470, r: 9, d: 0}, {x: 505, y: 420, r: 15, d: 3}, {x: 455, y: 360, r: 22, d: 6}];
  return (
    <div style={{position: "absolute", inset: 0, opacity: sale}}>
      ...
      <div style={{position: "absolute", left: 60, top: 235, width: 430, padding: "34px 36px", borderRadius: 120, background: "#fff",
        transform: `scale(${interpolate(p, [0, 1], [0.4, 1])})`, transformOrigin: "90% 100%", opacity: p,
...
const RelojTrabajo: React.FC<{id: string}> = ({id}) => <Reloj id={id} de="T1" a="LISTA" m0={19 * 60 + 30} m1={24 * 60 + 5 * 60 + 40} />;
...
const Encima: React.FC<{id: string}> = ({id}) => {
  switch (id) {
    case "P01": return <Cabecera hora="18:59" />;
    case "P02": return <><Cabecera hora="18:59" /><Pensamiento /></>;
    ...
    case "N22": return <><Cabecera hora="06:50" /><Dato texto="TURNO TERMINADO ✓" top={1560} /></>;
    ...
    case "N13": case "N14": return <Cabecera hora="09:05" />;
    default: return id.startsWith("T") ? <><RelojTrabajo id={id} /><HUD id={id} /></> : <Cabecera />;
  }
};
...
const SONIDO: Pista[] = [
  ...
  {src: fx("g-mm"), desde: en("N22", 30), vol: 0.6},
  {src: fx("marta-trrr"), desde: en("N22", 40), vol: 0.3},
  // 09:00
  ...
  {src: fx("chispa"), desde: en("N14", 10), vol: 0.8},
];

// Música: INTRIGA desde el primer cuadro (reloj, latido) hasta que se enciende el halo ·
// riser + coro en la iluminación · el DROP cae sobre el trabajo y corre entero hasta V1 LISTA
// (sting) · nada en el café (descansan) · una capa baja en el correo · el DING la mata · explosión seca.
const TENSION = "assets/gcl/cap02-v3/musica/temp-h-tension.wav";
const TRABAJO = "assets/gcl/cap02-v3/musica/temp-g-hype140.wav";
const TRAMOS_MUSICA: {src: string; desde: number; hasta: number; desdeS: number; vol: number}[] = [
  {src: TENSION, desde: 0, hasta: en("P06", 14), desdeS: 0.6, vol: 0.7},
  {src: TRABAJO, desde: en("N07", 10), hasta: en("LISTA", 6), desdeS: 2.5, vol: 0.6},
  {src: TRABAJO, desde: en("P17"), hasta: en("RESP", 6), desdeS: 0, vol: 0.3},
];

export const Cap02TurnoDeNocheV6: React.FC = () => {
  asegurarFuentes();
  return (
      ...
      {TRAMOS_MUSICA.map((m, i) => (
        <Sequence key={`m${i}`} from={m.desde} durationInFrames={m.hasta - m.desde}>
          <Audio src={staticFile(m.src)} startFrom={Math.round(m.desdeS * 30)}
            volume={(f) => m.vol * interpolate(f, [0, 6, m.hasta - m.desde - 4, m.hasta - m.desde], [0, 1, 1, 0], clamp)} />
        </Sequence>
      ))}
