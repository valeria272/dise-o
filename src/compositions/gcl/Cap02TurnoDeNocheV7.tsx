// ============================================================================
// G.CL / CAP.02 — «TURNO DE NOCHE» · CORTE 7 · «mañana lo veo» (últimos cambios de Valeria)
...
type Plano = {
  ...
  still?: boolean; grano?: boolean;
};
const PLANOS: Plano[] = [
  // HOOK — 18:59 · música de intriga desde el primer cuadro
  {id: "P01", dur: 33, zoom: {s: 1.0, x: "50%", y: "50%", s1: 1.08}},
  ...
  {id: "RESP", clip: "P17", dur: 54, desde: 4.2, vel: 0.15, grano: true},
  {id: "N16", dur: 60, desde: 0.2, vel: 1.6, grano: true},
  // ABAJO reciben el nuevo input → explotan → se le cae el halo
  {id: "BAJA", clip: "N02", dur: 12, desde: 0.3, vel: 3},
  {id: "LLEGA", clip: "N03", dur: 18, desde: 0.4, vel: 1.8},
  {id: "LEEN", clip: "N15", dur: 15, desde: 0.2, vel: 1.2},
  {id: "N13", dur: 48, desde: 1.9, vel: 1.8},
  {id: "N14", dur: 45, desde: 0.2},
  {id: "CIERRE", dur: CIERRE},
];
const INICIO: Record<string, number> = {};
{let t = 0; for (const p of PLANOS) {INICIO[p.id] = t; t += p.dur;}}
export const TURNO_V7_FRAMES = PLANOS.reduce((a, p) => a + p.dur, 0);
...
const NotaTubo: React.FC = () => {
  ...
  return (
    <div style={{position: "absolute", left: 150, right: 150, top: 1180, transform: `translateY(${(1 - p) * 60}px) rotate(-3deg)`, opacity: p,
      ...
      <div style={{fontSize: 46, fontWeight: 700, marginTop: 10}}>V1 lista.</div>
      ...
      <div style={{position: "absolute", inset: 0, transform: `translateY(${-vuela * 1400}px) scale(${1 - vuela * 0.3})`, opacity: 1 - vuela}}>
        <Ventana titulo="Nuevo mensaje">
          ...
          <div style={{marginTop: 22, display: "inline-flex", gap: 14, alignItems: "center", padding: "14px 22px", borderRadius: 14,
            background: "#FFE3EF", color: "#B0125E", fontSize: 30, fontWeight: 600}}>📎 campaña_V1_final.pdf</div>
...
const Encima: React.FC<{id: string}> = ({id}) => {
  switch (id) {
    ...
    case "T9": return <><RelojTrabajo id={id} /><HUD id={id} /><Mensaje de="MARTA → G" txt="brief archivado ✓" /></>;
    ...
    case "T14": return <><RelojTrabajo id={id} /><HUD id={id} /><Mensaje de="G → TODOS" txt="export… 98 %" /></>;
    case "LISTA": return <><Cabecera hora="05:41" /><HUD id={id} /><Sello texto="V1 LISTA ✓" /></>;
    ...
    case "LLEGA": case "LEEN": return <><Cabecera hora="09:05" /><Dato texto="NUEVO INPUT DEL CLIENTE" top={1500} /></>;
    case "N13": return <Cabecera hora="09:05" />;
    case "N14": return <><Cabecera hora="09:05" /><HaloCae /></>;
...
/** El halo de G se le cae después de la explosión (N14: casco centrado en x≈600, borde superior y≈816, piso y≈1460). */
const HaloCae: React.FC = () => {
  const f = useCurrentFrame();
  if (f < 10) return null;
  const t = f - 10;
  const y = 790 + Math.min(1, t / 14) * 670 + (t > 14 ? Math.abs(Math.sin((t - 14) / 4)) * -60 * Math.max(0, 1 - (t - 14) / 16) : 0);
  const rot = Math.min(75, t * 6);
  const op = interpolate(t, [0, 3], [0, 1], clamp);
  return (
    <div style={{position: "absolute", left: 600 - 95, top: y, width: 190, height: 60, borderRadius: "50%", border: "9px solid #FF2D8B",
      boxShadow: "0 0 24px rgba(255,45,139,0.9), inset 0 0 12px rgba(255,45,139,0.6)", opacity: op,
      transform: `rotate(${rot}deg) scaleY(${1 - 0.35 * Math.min(1, t / 14)})`}} />
  );
};
...
const SONIDO: Pista[] = [
  ...
  {src: fx("oficina-manana"), desde: en("P17"), hasta: en("BAJA"), vol: 0.4},
  ...
  {src: fx("whoosh"), desde: en("BAJA"), vol: 0.9},
  // abajo reciben y explotan
  {src: fx("tubo-baja"), desde: en("BAJA"), vol: 1},
  {src: fx("capsula-clonk"), desde: en("LLEGA", 3)},
  {src: fx("papel"), desde: en("LLEGA", 8), vol: 0.6},
  {src: fx("riser"), desde: en("LEEN"), vol: 0.9},
  {src: fx("g-glitch"), desde: en("LEEN", 6), vol: 0.6},
  {src: fx("explosion"), desde: en("N13", BOOM_F - 1)},
  {src: fx("papeles-caen"), desde: en("N14"), vol: 0.5},
  {src: fx("chispa"), desde: en("N14", 6), vol: 0.8},
  {src: fx("halo-clank"), desde: en("N14", 22), vol: 0.9},
  {src: fx("trombon-wah"), desde: en("N14", 26), vol: 0.8},
];

// Música de principio a fin (4 pistas, cross-fades de 8–12 f):
//   INTRIGA (temp-h) desde el primer cuadro hasta el halo · riser + coro en la iluminación ·
//   TRABAJO (temp-g, house 140 con build) del rayo de códigos hasta V1 LISTA, donde muere con el sting ·
//   DESCANSO (temp-j, lo-fi dorado) mientras sube, llega, duermen y brindan ·
//   OFICINA (temp-k, comedia de oficina con pizzicato) desde el sorbo de Pancho hasta el DING del cliente ·
//   silencio → riser → explosión seca → trombón «wah» → cierre con los ticks de la máquina.
const TENSION = "assets/gcl/cap02-v3/musica/temp-h-tension.wav";
const TRABAJO = "assets/gcl/cap02-v3/musica/temp-g-hype140.wav";
const DESCANSO = "assets/gcl/cap02-v3/musica/temp-j-descanso.wav";
const OFICINA = "assets/gcl/cap02-v3/musica/temp-k-oficina.wav";
const TRAMOS_MUSICA: {src: string; desde: number; hasta: number; desdeS: number; vol: number; fin?: number}[] = [
  {src: TENSION, desde: 0, hasta: en("P06", 16), desdeS: 0.6, vol: 0.7, fin: 12},
  {src: TRABAJO, desde: en("N07", 10), hasta: en("LISTA", 8), desdeS: 2.5, vol: 0.6, fin: 6},
  {src: DESCANSO, desde: en("N10"), hasta: en("N22", 60), desdeS: 0, vol: 0.45, fin: 10},
  {src: OFICINA, desde: en("P17"), hasta: en("RESP", 6), desdeS: 0.5, vol: 0.4, fin: 3},
];

export const Cap02TurnoDeNocheV7: React.FC = () => {
  asegurarFuentes();
  return (
      ...
      {TRAMOS_MUSICA.map((m, i) => (
        <Sequence key={`m${i}`} from={m.desde} durationInFrames={m.hasta - m.desde}>
          <Audio src={staticFile(m.src)} startFrom={Math.round(m.desdeS * 30)}
            volume={(f) => m.vol * interpolate(f, [0, 8, m.hasta - m.desde - (m.fin ?? 6), m.hasta - m.desde], [0, 1, 1, 0], clamp)} />
        </Sequence>
      ))}
