// ============================================================================
// G.CL / CAP.02 — «TURNO DE NOCHE» · CORTE 9 · «volvamos a la primera versión»
// 30 fps · 1080×1920 · 25-09-2026 · ~28,5 s de historia + cierre de serie
// ----------------------------------------------------------------------------
// Corte 9 = corte 8 revisado contra TODO lo decidido antes (Valeria: «te equivocaste, revisa bien»):
//   ⛔ sin Gin ni WhatsApp (eliminado en el corte 6) · ⛔ un solo «mm» · ✅ lo que dice el cliente
//   BAJA POR EL TUBO (candado 21) como nota en la cápsula: «¿más WOW?», «APROBADO ✓» y
//   «Mejor volvamos a la primera versión.» Sin nombres: es EL CLIENTE, vía Pancho, vía tubo.
...
const PLANOS: Plano[] = [
  ...
  // PRIMER GIRO — baja por el tubo · silencio · MÁS WOW · 2019
  {id: "BAJA1", clip: "N02", dur: 12, desde: 0.3, vel: 3},
  {id: "LLEGA1", clip: "N03", dur: 36, desde: 0.4, vel: 1.6},
  {id: "P09", dur: 24, desde: 0.3},
  ...
  {id: "MARTA", clip: "P04", dur: 15, desde: 0.05, vel: 0.15},   // Marta ni lo mira. Sin «mm»: ya sonó el único
  ...
  // FALSA VICTORIA — APROBADO baja por el tubo
  {id: "BAJA2", clip: "N02", dur: 10, desde: 0.3, vel: 3},
  {id: "LLEGA2", clip: "N03", dur: 24, desde: 0.4, vel: 1.6},
  {id: "P13", dur: 30, desde: 1.0, vel: 1.2},
  {id: "P14", dur: 24, desde: 1.8, vel: 2.4},
  // PICO — la última cápsula
  {id: "BAJA3", clip: "N02", dur: 12, desde: 0.3, vel: 3},
  {id: "LLEGA3", clip: "N03", dur: 42, desde: 0.4, vel: 1.4},
  {id: "P15", clip: "P09", dur: 18, desde: 2.0},
...
export const TURNO_V9_FRAMES = PLANOS.reduce((a, p) => a + p.dur, 0);
...
/** Lo que trae la cápsula: la nota del cliente (vía Pancho, vía tubo). Sin nombres, sin WhatsApp. */
const NotaCliente: React.FC<{hora: string; msj: string; grande?: boolean}> = ({hora, msj, grande}) => {
  const f = useCurrentFrame();
  const p = interpolate(f, [8, 16], [0, 1], clamp);
  return (
    <div style={{position: "absolute", left: 120, right: 120, top: 1180, transform: `translateY(${(1 - p) * 60}px) rotate(-2deg)`, opacity: p,
      background: "#F4EBD0", padding: "26px 34px", borderRadius: 6, boxShadow: "0 14px 40px rgba(0,0,0,0.55)", fontFamily: VOZ.data, color: "#2a2730"}}>
      <div style={{fontSize: 22, letterSpacing: 3, opacity: 0.7}}>CLIENTE → NIVEL −1 · {hora}</div>
      <div style={{fontSize: grande ? 60 : 44, fontWeight: 700, marginTop: 10, lineHeight: 1.15}}>{msj}</div>
    </div>
  );
};
...
const Encima: React.FC<{id: string}> = ({id}) => {
  switch (id) {
    ...
    case "BAJA1": case "BAJA2": case "BAJA3": return <Pisos />;
    case "LLEGA1": return <><Cabecera hora="02:31" /><NotaCliente hora="02:31" msj="¿Puede ser más WOW? 🙏" /></>;
    case "P09": return <Cabecera hora="02:31" />;
    case "LLEGA2": return <><Cabecera hora="06:40" /><NotaCliente hora="06:40" msj="APROBADO ✓" grande /></>;
    case "LLEGA3": return <><Cabecera hora="06:58" /><NotaCliente hora="06:58" msj="Mejor volvamos a la primera versión." /></>;
    ...
    case "P15": return <Cabecera hora="06:58" />;
...
const SONIDO: Pista[] = [
  ...
  // primer giro — baja por el tubo, silencio
  {src: fx("tubo-baja"), desde: en("BAJA1"), vol: 0.9},
  {src: fx("capsula-clonk"), desde: en("LLEGA1", 3)},
  {src: fx("papel"), desde: en("LLEGA1", 8), vol: 0.6},
  {src: fx("g-eh"), desde: en("P09", 8), vol: 0.8},
  ...
  // falsa victoria — APROBADO por el tubo
  {src: fx("tubo-baja"), desde: en("BAJA2"), vol: 0.9},
  {src: fx("capsula-clonk"), desde: en("LLEGA2", 3)},
  {src: fx("timbre-seco"), desde: en("P13")},
  {src: fx("wow-golpe"), desde: en("P13", 1), vol: 0.9},
  {src: fx("rolo-clonc"), desde: en("P14", 4), vol: 0.9},
  // pico — la última cápsula
  {src: fx("tubo-baja"), desde: en("BAJA3"), vol: 1},
  {src: fx("capsula-clonk"), desde: en("LLEGA3", 3)},
  {src: fx("papel"), desde: en("LLEGA3", 8), vol: 0.6},
...
const TRAMOS_MUSICA = [
  {desde: en("N07", 6), hasta: en("BAJA1"), desdeS: 5.0, vol: 0.62, fin: 2},
  {desde: en("E1"), hasta: en("LLEGA2", 12), desdeS: 8.0, vol: 0.66, fin: 2},
];

export const Cap02TurnoDeNocheV9: React.FC = () => {
  asegurarFuentes();
  return (
