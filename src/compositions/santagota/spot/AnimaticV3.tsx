/**
 * SANTA GOTA · «UNA GOTA. CAMBIA TODO.» — ANIMATIC V3 (última ronda de refinamiento, 15-09-2026 noche).
 *
...
export const T3 = {cliche: 0, gota: 60, plop: 96, cambia: 118, pizza: 150, sarten: 195, pasta: 249, reveal: 306, hero: 375,
...
const BOT = {tipX: 470, tipY: 600, h: 1480, rot: -100};   // la botella cruza el cuadro: nunca se ve entera ni flotando
const Reveal: React.FC = () => {
  const f = useCurrentFrame();            // 0..69
  const fS = SQUEEZE_FIN - T3.reveal, fG = GOTA_SUELTA - T3.reveal;
  const w = BOT.h / RATIO.s750;
  // cámara 1: macro del hilo → retroceso PARCIAL (z 2,2 → 1,35): boquilla, hombro y el arranque de la etiqueta; el resto queda fuera
  const k = interpolate(f, [0, 44], [0, 1], {easing: Easing.bezier(0.33, 0, 0.2, 1), extrapolateRight: "clamp"});
  const z = interpolate(k, [0, 1], [2.2, 1.35]);
  const px = interpolate(k, [0, 1], [BOT.tipX + 6, BOT.tipX + 330]);
  const py0 = interpolate(k, [0, 1], [BOT.tipY + 300, BOT.tipY - 40]);
  // la última gota se desprende en fG y cae; cámara 2 la sigue (tilt): la botella sale por arriba, el fondo ya es negro
...
const Hero: React.FC = () => {
  ...
  const d750 = Math.hypot(G1.x - 610, G1.y - (piso - 450)), d500 = Math.hypot(G1.x - 900, G1.y - (piso - 400));
  ...
  return (
    <Fondo>
      <div style={{position: "absolute", inset: 0, transform: `translateX(${trav}px) scale(${push})`, transformOrigin: "700px 760px"}}>
        ...
        <Botella src="hero4-500.png" ratio={RATIO.s500} cx={900} pisoY={piso - 58} h={720} z={10} blur={focoAtras} dim={rev1(d500) * (0.82 + luzOp * 0.3)} barrido={b1 >= 0 ? Math.min(1, b1 * 1.2) : b2} />
        <Botella src="hero4-750.png" ratio={RATIO.s750} cx={610} pisoY={piso} h={930} z={12} blur={focoDelante} dim={rev1(d750) * (1 + luzOp * 0.15)} barrido={b1 >= 0 ? b1 : b2} />
      ...
      <div style={{position: "absolute", right: SAFE_X, top: 548, zIndex: 20, textAlign: "right", fontFamily: SG.fonts.display, textTransform: "uppercase", color: "#FFFFFF", whiteSpace: "nowrap"}}>
        <Beat desde={fC1}>
          <div style={{fontSize: 48, fontWeight: 700, letterSpacing: "0.2em", lineHeight: 1}}>SOMOS LA</div>
          <div style={{fontSize: 112, fontWeight: 900, letterSpacing: "-0.02em", lineHeight: 0.95, color: C.lima, marginTop: 4}}>REVOLUCIÓN</div>
        </Beat>
        <Beat desde={fC2}>
          <div style={{fontSize: 50, fontWeight: 300, letterSpacing: "0.08em", lineHeight: 1.1, marginTop: 14}}>DEL ACEITE DE OLIVA.</div>
        </Beat>
      </div>
    </Fondo>
  );
...
export const AnimaticV3: React.FC = () => {
  useSpot();
  return (
    <AbsoluteFill style={{backgroundColor: "#000"}}>
      ...
      <Sequence from={T3.pasta} durationInFrames={T3.reveal - T3.pasta}><Pasta /></Sequence>
