/**
 * SANTA GOTA · «UNA SOLA GOTA… LO CAMBIA TODO.» — ANIMATIC V5 (segundo feedback de voz, 15-09-2026 noche).
 *
 * V5 sobre la V4:
 *   - Fuera el cliché (era casi igual a la gota): se ABRE con la gota, con más tiempo (0–2,8 s).
 *   - Copy en grande y explícito: «UNA SOLA GOTA / DE ACEITE DE OLIVA…» en el PLOP · «LO CAMBIA TODO.» en el cambio.
 *   - Escena con amigos más abierta y natural: plano amplio, la persona en plano medio, botella chica (foto compuesta).
 *   - Fuera la botella sola (inserto macro): de la mesa se corta a la gota que cae en el set → PLOP → la onda revela
 *     el PLACEMENT de los cuatro productos de una vez.
 *   - Cierre: claim en dos beats + «CÓMPRALO EN TODO CHILE · SANTAGOTA.CL» con el logo.
 *
 * (Cabecera de la V4:)
 *
 * V4 sobre la V3:
...
export const T3 = {cliche: 0, gota: 0, plop: 84, cambia: 108, pizza: 150, sarten: 195, pasta: 249, amigos: 306, reveal: 375, hero: 375,
...
const GotaLibre: React.FC = () => {
  const f = useCurrentFrame();            // 0..84 — abre el spot, con tiempo
  const k = 1920 / 2752;                  // la placa se ajusta al cuadro
  const box = {x: 1230 * k, y: 320 * k, w: 310 * k, h: 500 * k};
  const caida = interpolate(f, [14, 84], [0, 181], {easing: Easing.in(Easing.quad), extrapolateLeft: "clamp", extrapolateRight: "clamp"});   // cuelga un instante y cae; llega a la burrata en el corte
  const push = interpolate(f, [0, 84], [1.0, 1.07]);
  return (
    <Fondo color="#2a1d10">
  ...
  return (
    <Fondo color="#000">
      ...
      {f >= T3.plop && f < T3.pizza && (
        <Copy x={SAFE_X} y={470} size={150} lh={0.92}>
          <Beat desde={T3.plop}>UNA SOLA GOTA<br /><span style={{fontSize: 64, fontWeight: 300, letterSpacing: "0.08em"}}>DE ACEITE DE OLIVA…</span></Beat>
          <Beat desde={T3.cambia}><span style={{fontSize: 176, lineHeight: 0.9, display: "inline-block", marginTop: 18}}>LO CAMBIA <Lima>TODO.</Lima></span></Beat>
        </Copy>
      )}
    </Fondo>
  );
...
const AMIGOS_STILL = "assets/santagota/spot/mesa_still/0001.png";     // la mesa abierta compuesta (packshot oficial chico, pegado con puntos manuales)
const AMIGOS_MODO = "still" as "still" | "seq";
const AMIGOS_TIP = {x: 998, y: 736};                      // punta de la boquilla oficial en la mesa abierta (medida)
const Amigos: React.FC = () => {
  const f = useCurrentFrame();            // 0..69
  const idx = Math.min(120, Math.max(1, 1 + Math.round((f / 29.97) * 24)));
  const push = interpolate(f, [0, 69], [1.0, 1.06]);
  const trav = interpolate(f, [0, 69], [8, -8]);
  const flujo = (f * 3.4) % 42;
  const hiloW = 420; const hx = AMIGOS_TIP.x - 0.516 * hiloW;
  return (
    <Fondo>
      <div style={{position: "absolute", inset: 0, transform: `translateX(${trav}px) scale(${push})`, transformOrigin: "58% 45%"}}>
        ...
        {AMIGOS_MODO === "still" && (
          <Screen src={CAL3.hilo} x={hx} y={AMIGOS_TIP.y - 4 + flujo} w={hiloW} bright={0.9} op={0.7} z={7} sx={0.35} origen={`${0.516 * hiloW}px 0px`} clip={`inset(${flujo}px 0 30% 0)`} />
        )}
      </div>
...
// (el inserto macro de la botella sola se eliminó en la V5: «definitivamente no funciona»)
...
const Hero: React.FC = () => {
  ...
  const dCoc1 = Math.hypot(G1.x - 315, G1.y - (piso - 360)), dAde1 = Math.hypot(G1.x - 1000, G1.y - (piso - 350));
  ...
  return (
    <Fondo>
      <div style={{position: "absolute", inset: 0, transform: `translateX(${trav}px) scale(${push})`, transformOrigin: "660px 760px"}}>
        ...
        <Botella src="hero4-lata-cocinar.png" ratio={RATIO.lCoc} cx={315} pisoY={piso - 62} h={610} z={7} blur={1.6} dim={Math.max(rev1(dCoc1) * 0.9, descubre(dCoc))} refl={0.16} barrido={b2} />
        <Botella src="hero4-lata-aderezar.png" ratio={RATIO.lAde} cx={1000} pisoY={piso - 58} h={595} z={7} blur={1.6} dim={Math.max(rev1(dAde1) * 0.9, descubre(dAde))} refl={0.16} barrido={b2} />
      ...
      <div style={{position: "absolute", right: SAFE_X, top: 548, zIndex: 20, textAlign: "right", fontFamily: SG.fonts.display, textTransform: "uppercase", color: "#FFFFFF", whiteSpace: "nowrap"}}>
        ...
        <Beat desde={fP + 18}>
          <div style={{fontSize: 34, fontWeight: 700, letterSpacing: "0.16em", lineHeight: 1, marginTop: 44}}>CÓMPRALO EN TODO CHILE</div>
          <div style={{fontSize: 60, fontWeight: 900, letterSpacing: "0.02em", lineHeight: 1, marginTop: 8, color: C.lima}}>SANTAGOTA.CL</div>
        </Beat>
      </div>
    </Fondo>
  );
...
export const AnimaticV5: React.FC = () => {
  useSpot();
  return (
