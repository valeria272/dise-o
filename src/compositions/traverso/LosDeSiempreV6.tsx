/**
 * TRAVERSO × GRUPO COPYLAB — «LOS DE SIEMPRE» V6 · DIRECTOR'S CUT — quitar, conectar y acelerar (19,6 s · 24 fps)
 * ------------------------------------------------------------------------------------------
 ...
 *
 * Música: audio/banda-v6.mp3 = A-garage-1 desde 1,52 s, sin empalmes; sólo se vacía 9,1–9,5.
...
export const V6_FPS = 24;
export const V6_W = 1080;
export const V6_H = 1920;
export const V6_DURATION = Math.round(19.6 * V6_FPS); // 470
const A = "assets/traverso/lds2";
const F = (s: number) => Math.round(s * V6_FPS);
...
// Eventos MEDIDOS de audio/banda-v6.mp3 (la misma canción, offset 4,42): 1,04 · 1,94 · 2,42 · 3,14 ·
...
const T = {r1: 1.5, r2: 1.94, r3: 2.42, walkBack: 3.14, trio: 3.58, vacio: 6.15, reveal: 6.6, destino: 9.36, cruce: 11.58, mesa: 12.58, end: 17.08, fin: 19.6};
...
const PLANOS: Plano[] = [
  ...
  {id: "reveal", from: T.reveal, to: T.destino, src: "c09.mp4", trim: 0.0, rate: 1.75, push: [1.22, 1.0], origin: "50% 45%", shake: true},
  // LOS DE SIEMPRE → packaging → CUT directo a GRUPO COPYLAB (UN solo plano desde atrás)
  {id: "destino", from: T.destino, to: T.cruce, src: "c13.mp4", trim: 0.3, rate: 2.0, burn: true},
  // cruzan: Ketchup tapa la lente → wipe → ya sentados
  {id: "cruce", from: T.cruce, to: T.mesa, src: "c14.mp4", trim: 3.9, rate: 1.0, fromWhite: true, wipeOut: true},
  {id: "mesa", from: T.mesa, to: T.end, src: "c15.mp4", trim: 0.3, rate: 1.0, push: [1.0, 1.05], origin: "55% 40%"},
];
...
const SFX: {src: string; at: number; vol: number; dur?: number; base?: string}[] = [
  ...
  {src: "sfx-impacto.mp3", at: T.destino, vol: 0.5}, {src: "sfx-pasos.mp3", at: T.destino, vol: 0.4, dur: 2.0, base: "lds"}, {src: "sfx-riser.mp3", at: T.cruce - 1.0, vol: 0.6}, {src: "sfx-puerta.mp3", at: T.cruce - 0.4, vol: 0.55, base: "lds"},
  ...
  {src: "sfx-bass.mp3", at: 19.08, vol: 0.85},
];

export const LosDeSiempreV6: React.FC = () => {
  void fontPromise;
  return (
    <AbsoluteFill style={{background: INK}}>
      ...
      <Sequence from={F(13.5)} durationInFrames={F(T.end - 13.5)} layout="none"><Titular lineas={["Primera reunión."]} size={90} bottom={300} /></Sequence>
      ...
      <Sequence from={F(T.end)} durationInFrames={F(T.fin - T.end)} layout="none">
        ...
        <Sequence from={F(1.1)} layout="none">
