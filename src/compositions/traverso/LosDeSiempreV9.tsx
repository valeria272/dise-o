/**
 * TRAVERSO × GRUPO COPYLAB — «LOS DE SIEMPRE» V9 · FINAL CUT — sacar redundancias (19,6 s · 24 fps)
 * ------------------------------------------------------------------------------
...
export const V9_FPS = 24;
export const V9_W = 1080;
export const V9_H = 1920;
export const V9_DURATION = Math.round(19.6 * V9_FPS); // 470
const A = "assets/traverso/lds2";
const F = (s: number) => Math.round(s * V9_FPS);
...
// CAMINAN → MACRO → YA DETENIDOS (semisilueta, expectativa) → CLACK izq 3,58 · centro 4,04 · der 4,74 (onsets reales)
// → LOS DE SIEMPRE. → solapas → vacío 6,15–6,6 → DROP 6,6 (Traverso) → 9,36 GRUPO COPYLAB → 11,58 luz → YA SENTADOS
const T = {walk: 0.0, macro: 1.62, stop: 2.42, foco1: 3.58, foco2: 4.04, foco3: 4.74, vacio: 6.15, reveal: 6.6,
  destino: 9.36, mesa: 11.58, end: 16.5, card2: 17.6, fin: 19.6};
...
const PLANOS: Plano[] = [
  ...
  {id: "stage", from: T.stop, to: T.reveal, src: "c08.mp4", trim: 0.2, rate: 1.0, push: [1.0, 1.05], origin: "50% 32%", stage: true},
...
const SFX: {src: string; at: number; vol: number; dur?: number; base?: string}[] = [
  {src: "sfx-pasos.mp3", at: 0.0, vol: 0.45, dur: 1.6, base: "lds"},
  ...
  {src: "sfx-tela.mp3", at: 5.4, vol: 0.4, base: "lds"},
  ...
  {src: "sfx-camara.mp3", at: T.mesa - 0.06, vol: 0.5}, {src: "sfx-oficina.mp3", at: T.mesa, vol: 0.28, dur: 4.9, base: "lds"}, {src: "sfx-carpeta.mp3", at: T.mesa + 0.8, vol: 0.45},
  {src: "sfx-bass.mp3", at: 19.08, vol: 0.9},
];

export const LosDeSiempreV9: React.FC = () => {
  void fontPromise;
  return (
    <AbsoluteFill style={{background: INK}}>
      ...
      <Sequence from={F(12.6)} durationInFrames={F(T.end - 12.6)} layout="none"><Titular lineas={["Primera reunión."]} size={84} bottom={320} /></Sequence>
      <Sequence from={F(14.58)} durationInFrames={F(T.end - 14.58)} layout="none"><Titular lineas={["Nadie necesita", "presentarlos."]} size={60} color={MOSTAZA} bottom={190} wght={650} /></Sequence>
      ...
      <Sequence from={F(T.end)} durationInFrames={F(T.fin - T.end)} layout="none">
        ...
        <Sequence from={F(T.card2 - T.end)} layout="none">
          <Titular lineas={["Bienvenidos, Traverso."]} size={72} wdth={70} wght={750} bottom={1080} />
          <Marcas />
        </Sequence>
      </Sequence>
