...
const ADMIN = new Set([
  "piece_add", "piece_edit", "piece_img", "piece_del", "comment_resolve", "piece_fmt", "archivar", "piece_slides",
  ...
  try {
    ...
    if (type === "approve") {
      ...
      const p = {
        ...
        img: clip(b.img, 500) || null, kind: (b.kind === "pdf" ? "pdf" : "img"),
        imgs: Array.isArray(b.imgs) ? b.imgs.slice(0, 20).map((u) => clip(u, 500)) : null,
        v: 1, estado: "pendiente", comments: [], history: [],
      };
      if (p.imgs && p.imgs.length) p.img = p.img || p.imgs[0];
      log(p, "Pieza cargada (V1)" + (p.imgs && p.imgs.length > 1 ? " — carrusel de " + p.imgs.length + " slides" : ""));
      S.pieces.push(p);
      ...
      if (b.kind) x.kind = (b.kind === "pdf" ? "pdf" : "img");
      x.v = (x.v || 1) + 1;
      x.estado = "corregida";
    ...
    } else if (type === "piece_slides") {
      const x = find(S.pieces, b.id); if (!x) throw new Error("pieza no existe");
      const nuevas = Array.isArray(b.imgs) ? b.imgs.slice(0, 20).map((u) => clip(u, 500)) : [];
      if (!nuevas.length) throw new Error("no llegaron slides");
      if (b.modo === "reemplazar") {
        ...
        x.estado = "corregida";
        (x.comments || []).forEach((c) => { if (!c.res) { c.res = true; c.resEn = "V" + x.v; } });
        log(x, "🎨 Carrusel corregido: V" + x.v + " (" + nuevas.length + " slides)" + (b.nota ? " — " + clip(b.nota, 200) : "") + ". Comentarios anteriores marcados resueltos.");
      } else {
        x.imgs = (x.imgs && x.imgs.length ? x.imgs : (x.img ? [x.img] : [])).concat(nuevas).slice(0, 20);
        x.img = x.img || x.imgs[0];
        log(x, "Slides agregadas: " + nuevas.length + " (total " + x.imgs.length + ")");
      }
